...
# AWS S3 support (optional - falls back to local if not configured)
try:
    import boto3
    S3_ENABLED = True
except ImportError:
...
S3_BUCKET = os.getenv('S3_BUCKET', 'overfitting-demo-data')
AWS_REGION = os.getenv('AWS_REGION', 'us-east-1')

# Initialize S3 client if enabled
if USE_S3 and S3_ENABLED:
...
def load_from_s3(key, file_type='csv'):
    """Load file from S3"""
    try:
...
def load_from_local(filepath, file_type='csv'):
...
def load_training_performance():
    """Load all strategies performance on training data"""
    if USE_S3:
        return load_from_s3('results/training_performance.csv', 'csv')
    else:
        return load_from_local('results/training_performance.csv', 'csv')

@st.cache_data
def load_best_strategy():
    """Load the best performing strategy"""
    if USE_S3:
        return load_from_s3('results/best_strategy.json', 'json')
    else:
        return load_from_local('results/best_strategy.json', 'json')

@st.cache_data
def load_validation_results():
    """Load validation results"""
    if USE_S3:
        return load_from_s3('results/validation_results.json', 'json')
    else:
        return load_from_local('results/validation_results.json', 'json')

@st.cache_data
def load_validation_summary():
    """Load validation summary for top strategies"""
    if USE_S3:
        return load_from_s3('results/validation_summary.json', 'json')
    else:
        return load_from_local('results/validation_summary.json', 'json')

@st.cache_data
def load_plot_image(plot_name):
    """Load plot image from S3 or local"""
    if USE_S3:
        return load_from_s3(f'plots/{plot_name}', 'image')
    else:
        try:
            with open(f'plots/{plot_name}', 'rb') as f:
                return f.read()
        except FileNotFoundError:
            return None
            return json.load(f)
